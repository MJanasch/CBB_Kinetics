...
function [dfodc,ParameterSet] = MJanasch_Calculate_DFODC(N,ParameterSet,V_K_Indeces,Fluxes,SFull)
...
%% Define Metabolite Concentrations
    ...
    ATP         = N.species(20).initialConcentration;
    NADP        = N.species(21).initialConcentration;
    O2          = N.species(22).initialConcentration;
    CO2_cax     = N.species(23).initialConcentration;
    CO2_cyt     = N.species(24).initialConcentration;
    ACCOA       = N.species(25).initialConcentration;
    NADH        = N.species(26).initialConcentration;
    NAD         = N.species(27).initialConcentration;
    COA         = N.species(28).initialConcentration;
    BioMass_R5P = N.species(29).initialConcentration;
    BioMass_F6P = N.species(30).initialConcentration;
    BioMass_E4P = N.species(31).initialConcentration;
    BioMass_PEP = N.species(32).initialConcentration;
    BioMass_PYR = N.species(33).initialConcentration;
    BioMass_P3G = N.species(34).initialConcentration;
    PPool       = N.species(35).initialConcentration;
...
% Set Vmax's and K to 1, if not already
    ...
    KmADPv19    = ParameterSet(114);
    KmPHIv19    = ParameterSet(115);
    KmATPv19    = ParameterSet(116);
    K_eqv19     = ParameterSet(117);
    
    V_maxv20    = ParameterSet(118);
    KmNADPv20   = ParameterSet(119);
    KmNADPHv20  = ParameterSet(120);
    K_eqv20     = ParameterSet(121);
    
    V_maxv21    = ParameterSet(122);
    KmR5Pv21    = ParameterSet(123);
    
    V_maxv22    = ParameterSet(124);
    KmF6Pv22    = ParameterSet(125);
    
    V_maxv23    = ParameterSet(126);
    KmE4Pv23    = ParameterSet(127);
    
    V_maxv24    = ParameterSet(128);
    KmPEPv24    = ParameterSet(129);

    V_maxv25    = ParameterSet(130);
    KmPYRv25    = ParameterSet(131);

    V_maxv26    = ParameterSet(132);
    KmP3Gv26    = ParameterSet(133);
    
    K_PPoolv27  = ParameterSet(134);
...
%% Rate Equations    
    ...
    pippo(19,1)= V_maxv19*(ADP*PHI-ATP/K_eqv19)/(KmADPv19*KmPHIv19*(1+ADP/KmADPv19+PHI/KmPHIv19+ATP/KmATPv19+(ADP*PHI)/(KmADPv19*KmPHIv19)));
    pippo(20,1)= V_maxv20 * (NADP / KmNADPv20) * (1 - NADPH / NADP / K_eqv20) / (1 + NADP / KmNADPv20 + NADPH / KmNADPHv20);
    
    pippo(21,1)= V_maxv21 * R5P/(KmR5Pv21 + R5P);
    pippo(22,1)= V_maxv22*F6P/(KmF6Pv22+F6P);
    pippo(23,1)= V_maxv23*E4P/(KmE4Pv23+E4P);
    pippo(24,1)= V_maxv24*PEP/(KmPEPv24+PEP);
    pippo(25,1)= V_maxv25*PYR/(KmPYRv25+PYR);
    pippo(26,1)= V_maxv26*P3G/(KmP3Gv26+P3G);
    pippo(27,1)= K_PPoolv27*(PPool - PHI);    
...
%% Calculate Vmax and K
    ...
    K_PPoolv27  = ParameterSet(V_K_Indeces(27));
...
dfodc(3,26) = V_maxv26/(KmP3Gv26 + P3G) - (P3G*V_maxv26)/(KmP3Gv26 + P3G)^2;
...
dfodc(4,19) = (PHI*V_maxv19)/(KmADPv19*KmPHIv19*(ADP/KmADPv19 + ATP/KmATPv19 + PHI/KmPHIv19 + (ADP*PHI)/(KmADPv19*KmPHIv19) + 1)) + (V_maxv19*(1/KmADPv19 + PHI/(KmADPv19*KmPHIv19))*(ATP/K_eqv19 - ADP*PHI))/(KmADPv19*KmPHIv19*(ADP/KmADPv19 + ATP/KmATPv19 + PHI/KmPHIv19 + (ADP*PHI)/(KmADPv19*KmPHIv19) + 1)^2);
...
dfodc(6,23) = V_maxv23/(E4P + KmE4Pv23) - (E4P*V_maxv23)/(E4P + KmE4Pv23)^2;
...
dfodc(7,11) = 0;
dfodc(7,22) = V_maxv22/(F6P + KmF6Pv22) - (F6P*V_maxv22)/(F6P + KmF6Pv22)^2;
...
dfodc(8,10) = (DHAP*E4P*V_maxv10*(SBP/(DHAP*E4P*K_eqv10) - 1)*(SBP/KmSBPv10 + 1))/(KmDHAPv10*KmE4Pv10*KmFBPv10*((FBP/KmFBPv10 + 1)*(SBP/KmSBPv10 + 1) + (DHAP/KmDHAPv10 + 1)*(E4P/KmE4Pv10 + 1)*(GAP/KmGAPv10 + 1) - 1)^2);
...
dfodc(10,20)= (NADP*V_maxv20*(NADPH/(K_eqv20*NADP) - 1))/(KmNADPv20*KmNADPHv20*(NADP/KmNADPv20 + NADPH/KmNADPHv20 + 1)^2) - V_maxv20/(K_eqv20*KmNADPv20*(NADP/KmNADPv20 + NADPH/KmNADPHv20 + 1));
...
dfodc(11,24)= V_maxv24/(KmPEPv24 + PEP) - (PEP*V_maxv24)/(KmPEPv24 + PEP)^2;
...
dfodc(12,19)= (ADP*V_maxv19)/(KmADPv19*KmPHIv19*(ADP/KmADPv19 + ATP/KmATPv19 + PHI/KmPHIv19 + (ADP*PHI)/(KmADPv19*KmPHIv19) + 1)) + (V_maxv19*(1/KmPHIv19 + ADP/(KmADPv19*KmPHIv19))*(ATP/K_eqv19 - ADP*PHI))/(KmADPv19*KmPHIv19*(ADP/KmADPv19 + ATP/KmATPv19 + PHI/KmPHIv19 + (ADP*PHI)/(KmADPv19*KmPHIv19) + 1)^2);
dfodc(12,27)= -K_PPoolv27;
...
dfodc(13,21)= V_maxv21/(KmR5Pv21 + R5P) - (R5P*V_maxv21)/(KmR5Pv21 + R5P)^2;
...
dfodc(19,25)= V_maxv25/(KmPYRv25 + PYR) - (PYR*V_maxv25)/(KmPYRv25 + PYR)^2;

% Row 20
dfodc(20,2) = (ATP*P3G*V_maxv2*((ADP*BPG)/(ATP*K_eqv2*P3G) - 1))/(KmATPv2^2*KmP3Gv2*(BPG/KmBPGv2 + P3G/KmP3Gv2 + 1)*(ADP/KmADPv2 + ATP/KmATPv2 + 1)^2) - (P3G*V_maxv2*((ADP*BPG)/(ATP*K_eqv2*P3G) - 1))/(KmATPv2*KmP3Gv2*(BPG/KmBPGv2 + P3G/KmP3Gv2 + 1)*(ADP/KmADPv2 + ATP/KmATPv2 + 1)) + (ADP*BPG*V_maxv2)/(ATP*K_eqv2*KmATPv2*KmP3Gv2*(BPG/KmBPGv2 + P3G/KmP3Gv2 + 1)*(ADP/KmADPv2 + ATP/KmATPv2 + 1));
dfodc(20,9) = (ATP*F6P*KiATPv9*V_maxv9*((ADP*FBP)/(ATP*F6P*K_eqv9) - 1))/(KmATPv9*KmF6Pv9*(ATP + KiATPv9)^2*(F6P/KmF6Pv9 + FBP/KmFBPv9 + 1)*(ADP/KmADPv9 + ATP/KmATPv9 + 1)) - (F6P*KiATPv9*V_maxv9*((ADP*FBP)/(ATP*F6P*K_eqv9) - 1))/(KmATPv9*KmF6Pv9*(ATP + KiATPv9)*(F6P/KmF6Pv9 + FBP/KmFBPv9 + 1)*(ADP/KmADPv9 + ATP/KmATPv9 + 1)) + (ATP*F6P*KiATPv9*V_maxv9*((ADP*FBP)/(ATP*F6P*K_eqv9) - 1))/(KmATPv9^2*KmF6Pv9*(ATP + KiATPv9)*(F6P/KmF6Pv9 + FBP/KmFBPv9 + 1)*(ADP/KmADPv9 + ATP/KmATPv9 + 1)^2) + (ADP*FBP*KiATPv9*V_maxv9)/(ATP*K_eqv9*KmATPv9*KmF6Pv9*(ATP + KiATPv9)*(F6P/KmF6Pv9 + FBP/KmFBPv9 + 1)*(ADP/KmADPv9 + ATP/KmATPv9 + 1));
dfodc(20,14)= (ATP*KiADPv14*KiPEPv14*Ru5P*V_maxv14*((ADP*RuBP)/(ATP*K_eqv14*Ru5P) - 1))/(KmATPv14^2*KmRu5Pv14*(ADP + KiADPv14)*(KiPEPv14 + PEP)*(Ru5P/KmRu5Pv14 + RuBP/KmRuBPv14 + 1)*(ADP/KmADPv14 + ATP/KmATPv14 + 1)^2) - (KiADPv14*KiPEPv14*Ru5P*V_maxv14*((ADP*RuBP)/(ATP*K_eqv14*Ru5P) - 1))/(KmATPv14*KmRu5Pv14*(ADP + KiADPv14)*(KiPEPv14 + PEP)*(Ru5P/KmRu5Pv14 + RuBP/KmRuBPv14 + 1)*(ADP/KmADPv14 + ATP/KmATPv14 + 1)) + (ADP*KiADPv14*KiPEPv14*RuBP*V_maxv14)/(ATP*K_eqv14*KmATPv14*KmRu5Pv14*(ADP + KiADPv14)*(KiPEPv14 + PEP)*(Ru5P/KmRu5Pv14 + RuBP/KmRuBPv14 + 1)*(ADP/KmADPv14 + ATP/KmATPv14 + 1));
dfodc(20,17)= (ADP*KiATPv17*KiFBPv17*KiPHIv17*PEP*R5P*V_maxv17*((ATP*PYR)/(ADP*K_eqv17*PEP) - 1))/(KmADPv17*KmPEPv17*(ATP + KiATPv17)^2*(FBP + KiFBPv17)*(KiPHIv17 + PHI)*(KaR5Pv17 + R5P)*(PEP/KmPEPv17 + PYR/KmPYRv17 + 1)*(ADP/KmADPv17 + ATP/KmATPv17 + 1)) - (KiATPv17*KiFBPv17*KiPHIv17*PYR*R5P*V_maxv17)/(K_eqv17*KmADPv17*KmPEPv17*(ATP + KiATPv17)*(FBP + KiFBPv17)*(KiPHIv17 + PHI)*(KaR5Pv17 + R5P)*(PEP/KmPEPv17 + PYR/KmPYRv17 + 1)*(ADP/KmADPv17 + ATP/KmATPv17 + 1)) + (ADP*KiATPv17*KiFBPv17*KiPHIv17*PEP*R5P*V_maxv17*((ATP*PYR)/(ADP*K_eqv17*PEP) - 1))/(KmADPv17*KmATPv17*KmPEPv17*(ATP + KiATPv17)*(FBP + KiFBPv17)*(KiPHIv17 + PHI)*(KaR5Pv17 + R5P)*(PEP/KmPEPv17 + PYR/KmPYRv17 + 1)*(ADP/KmADPv17 + ATP/KmATPv17 + 1)^2);
dfodc(20,19)= (V_maxv19*(ATP/K_eqv19 - ADP*PHI))/(KmADPv19*KmATPv19*KmPHIv19*(ADP/KmADPv19 + ATP/KmATPv19 + PHI/KmPHIv19 + (ADP*PHI)/(KmADPv19*KmPHIv19) + 1)^2) - V_maxv19/(K_eqv19*KmADPv19*KmPHIv19*(ADP/KmADPv19 + ATP/KmATPv19 + PHI/KmPHIv19 + (ADP*PHI)/(KmADPv19*KmPHIv19) + 1));
...
dfodc(21,20)= (NADP*V_maxv20*(NADPH/(K_eqv20*NADP) - 1))/(KmNADPv20^2*(NADP/KmNADPv20 + NADPH/KmNADPHv20 + 1)^2) - (V_maxv20*(NADPH/(K_eqv20*NADP) - 1))/(KmNADPv20*(NADP/KmNADPv20 + NADPH/KmNADPHv20 + 1)) + (NADPH*V_maxv20)/(K_eqv20*KmNADPv20*NADP*(NADP/KmNADPv20 + NADPH/KmNADPHv20 + 1));

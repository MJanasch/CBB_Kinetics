...
function [dfodc,ParameterSet] = MJanasch_Calculate_DFODC_XFPK(N,ParameterSet,V_K_Indeces,Fluxes,SFull)
...
ParameterSet(122) = ParameterSet(114); % KmF6P
ParameterSet(123) = ParameterSet(115); % KmPHI
ParameterSet(124) = ParameterSet(116); % KmE4P
ParameterSet(125) = ParameterSet(117); % KmACETP
ParameterSet(126) = ParameterSet(118); % KmXu5P
ParameterSet(127) = ParameterSet(119); % KmGAP
...
% Set Vmax's and K to 1, if not already
    ...
    V_maxv20    = ParameterSet(121);
    KmF6Pv20    = ParameterSet(122); % like KmF6Pv19 
    KmPHIv20    = ParameterSet(123); % like KmPHIv19
    KmE4Pv20    = ParameterSet(124); % like KmE4Pv19
    KmACETPv20  = ParameterSet(125); % like KmACETPv19
    KmXu5Pv20   = ParameterSet(126); % like KmXu5Pv19
    KmGAPv20    = ParameterSet(127); % like KmGAPv19
    K_eqv20     = ParameterSet(128);
    
    V_maxv21    = ParameterSet(129);
    KmACETPv21  = ParameterSet(130);
    KmCOAv21    = ParameterSet(131);
    KmACCOAv21  = ParameterSet(132);
    KmPHIv21    = ParameterSet(133);
    K_eqv21     = ParameterSet(134);
    
    V_maxv22    = ParameterSet(135);
    KmADPv22    = ParameterSet(136);
    KmPHIv22    = ParameterSet(137);
    KmATPv22    = ParameterSet(138);
    K_eqv22     = ParameterSet(139);
    
    V_maxv23    = ParameterSet(140);
    KmNADPv23   = ParameterSet(141);
    KmNADPHv23  = ParameterSet(142);
    K_eqv23     = ParameterSet(143);
    
    V_maxv24    = ParameterSet(144);
    KmR5Pv24    = ParameterSet(145);
    
    V_maxv25    = ParameterSet(146);
    KmF6Pv25    = ParameterSet(147);
    
    V_maxv26    = ParameterSet(148);
    KmE4Pv26    = ParameterSet(149);
    
    V_maxv27    = ParameterSet(150);
    KmPEPv27    = ParameterSet(151);

    V_maxv28    = ParameterSet(152);
    KmPYRv28    = ParameterSet(153);

    V_maxv29    = ParameterSet(154);
    KmP3Gv29    = ParameterSet(155);
    
    K_PPoolv30  = ParameterSet(156);
...
%% Rate Equations    
    ...
    pippo(19,1)= V_maxv19*(F6P*PHI/(KmF6Pv19*KmPHIv19))*(1-E4P*ACETP/(F6P*PHI)/K_eqv19)/((1+F6P/KmF6Pv19+E4P/KmE4Pv19)*(1+PHI/KmPHIv19+ACETP/KmACETPv19)*(1+Xu5P/KmXu5Pv19+GAP/KmGAPv19));
    pippo(20,1)= V_maxv20*(Xu5P*PHI/(KmXu5Pv20*KmPHIv20))*(1-GAP*ACETP/(Xu5P*PHI)/K_eqv20)/((1+F6P/KmF6Pv20+E4P/KmE4Pv20)*(1+PHI/KmPHIv20+ACETP/KmACETPv20)*(1+Xu5P/KmXu5Pv20+GAP/KmGAPv20));
...
dfodc(6,19) = (F6P*PHI*V_maxv19*((ACETP*E4P)/(F6P*K_eqv19*PHI) - 1))/(KmE4Pv19*KmF6Pv19*KmPHIv19*(ACETP/KmACETPv19 + PHI/KmPHIv19 + 1)*(GAP/KmGAPv19 + Xu5P/KmXu5Pv19 + 1)*(E4P/KmE4Pv19 + F6P/KmF6Pv19 + 1)^2) - (ACETP*V_maxv19)/(K_eqv19*KmF6Pv19*KmPHIv19*(ACETP/KmACETPv19 + PHI/KmPHIv19 + 1)*(GAP/KmGAPv19 + Xu5P/KmXu5Pv19 + 1)*(E4P/KmE4Pv19 + F6P/KmF6Pv19 + 1));
dfodc(6,20) = (PHI*V_maxv20*Xu5P*((ACETP*GAP)/(K_eqv20*PHI*Xu5P) - 1))/(KmE4Pv20*KmPHIv20*KmXu5Pv20*(ACETP/KmACETPv20 + PHI/KmPHIv20 + 1)*(GAP/KmGAPv20 + Xu5P/KmXu5Pv20 + 1)*(E4P/KmE4Pv20 + F6P/KmF6Pv20 + 1)^2);
...
dfodc(7,11) = 0;
dfodc(7,19) = (F6P*PHI*V_maxv19*((ACETP*E4P)/(F6P*K_eqv19*PHI) - 1))/(KmF6Pv19^2*KmPHIv19*(ACETP/KmACETPv19 + PHI/KmPHIv19 + 1)*(GAP/KmGAPv19 + Xu5P/KmXu5Pv19 + 1)*(E4P/KmE4Pv19 + F6P/KmF6Pv19 + 1)^2) - (PHI*V_maxv19*((ACETP*E4P)/(F6P*K_eqv19*PHI) - 1))/(KmF6Pv19*KmPHIv19*(ACETP/KmACETPv19 + PHI/KmPHIv19 + 1)*(GAP/KmGAPv19 + Xu5P/KmXu5Pv19 + 1)*(E4P/KmE4Pv19 + F6P/KmF6Pv19 + 1)) + (ACETP*E4P*V_maxv19)/(F6P*K_eqv19*KmF6Pv19*KmPHIv19*(ACETP/KmACETPv19 + PHI/KmPHIv19 + 1)*(GAP/KmGAPv19 + Xu5P/KmXu5Pv19 + 1)*(E4P/KmE4Pv19 + F6P/KmF6Pv19 + 1));
dfodc(7,20) = (PHI*V_maxv20*Xu5P*((ACETP*GAP)/(K_eqv20*PHI*Xu5P) - 1))/(KmF6Pv20*KmPHIv20*KmXu5Pv20*(ACETP/KmACETPv20 + PHI/KmPHIv20 + 1)*(GAP/KmGAPv20 + Xu5P/KmXu5Pv20 + 1)*(E4P/KmE4Pv20 + F6P/KmF6Pv20 + 1)^2);
...
dfodc(9,19) = (F6P*PHI*V_maxv19*((ACETP*E4P)/(F6P*K_eqv19*PHI) - 1))/(KmGAPv19*KmF6Pv19*KmPHIv19*(ACETP/KmACETPv19 + PHI/KmPHIv19 + 1)*(GAP/KmGAPv19 + Xu5P/KmXu5Pv19 + 1)^2*(E4P/KmE4Pv19 + F6P/KmF6Pv19 + 1));
dfodc(9,20) = (PHI*V_maxv20*Xu5P*((ACETP*GAP)/(K_eqv20*PHI*Xu5P) - 1))/(KmGAPv20*KmPHIv20*KmXu5Pv20*(ACETP/KmACETPv20 + PHI/KmPHIv20 + 1)*(GAP/KmGAPv20 + Xu5P/KmXu5Pv20 + 1)^2*(E4P/KmE4Pv20 + F6P/KmF6Pv20 + 1)) - (ACETP*V_maxv20)/(K_eqv20*KmPHIv20*KmXu5Pv20*(ACETP/KmACETPv20 + PHI/KmPHIv20 + 1)*(GAP/KmGAPv20 + Xu5P/KmXu5Pv20 + 1)*(E4P/KmE4Pv20 + F6P/KmF6Pv20 + 1));
...
dfodc(12,19)= (F6P*PHI*V_maxv19*((ACETP*E4P)/(F6P*K_eqv19*PHI) - 1))/(KmF6Pv19*KmPHIv19^2*(ACETP/KmACETPv19 + PHI/KmPHIv19 + 1)^2*(GAP/KmGAPv19 + Xu5P/KmXu5Pv19 + 1)*(E4P/KmE4Pv19 + F6P/KmF6Pv19 + 1)) - (F6P*V_maxv19*((ACETP*E4P)/(F6P*K_eqv19*PHI) - 1))/(KmF6Pv19*KmPHIv19*(ACETP/KmACETPv19 + PHI/KmPHIv19 + 1)*(GAP/KmGAPv19 + Xu5P/KmXu5Pv19 + 1)*(E4P/KmE4Pv19 + F6P/KmF6Pv19 + 1)) + (ACETP*E4P*V_maxv19)/(K_eqv19*KmF6Pv19*KmPHIv19*PHI*(ACETP/KmACETPv19 + PHI/KmPHIv19 + 1)*(GAP/KmGAPv19 + Xu5P/KmXu5Pv19 + 1)*(E4P/KmE4Pv19 + F6P/KmF6Pv19 + 1));
dfodc(12,20)= (PHI*V_maxv20*Xu5P*((ACETP*GAP)/(K_eqv20*PHI*Xu5P) - 1))/(KmPHIv20^2*KmXu5Pv20*(ACETP/KmACETPv20 + PHI/KmPHIv20 + 1)^2*(GAP/KmGAPv20 + Xu5P/KmXu5Pv20 + 1)*(E4P/KmE4Pv20 + F6P/KmF6Pv20 + 1)) - (V_maxv20*Xu5P*((ACETP*GAP)/(K_eqv20*PHI*Xu5P) - 1))/(KmPHIv20*KmXu5Pv20*(ACETP/KmACETPv20 + PHI/KmPHIv20 + 1)*(GAP/KmGAPv20 + Xu5P/KmXu5Pv20 + 1)*(E4P/KmE4Pv20 + F6P/KmF6Pv20 + 1)) + (ACETP*GAP*V_maxv20)/(K_eqv20*KmPHIv20*KmXu5Pv20*PHI*(ACETP/KmACETPv20 + PHI/KmPHIv20 + 1)*(GAP/KmGAPv20 + Xu5P/KmXu5Pv20 + 1)*(E4P/KmE4Pv20 + F6P/KmF6Pv20 + 1));
...
dfodc(18,19)= (F6P*PHI*V_maxv19*((ACETP*E4P)/(F6P*K_eqv19*PHI) - 1))/(KmF6Pv19*KmPHIv19*KmXu5Pv19*(ACETP/KmACETPv19 + PHI/KmPHIv19 + 1)*(GAP/KmGAPv19 + Xu5P/KmXu5Pv19 + 1)^2*(E4P/KmE4Pv19 + F6P/KmF6Pv19 + 1));
dfodc(18,20)= (PHI*V_maxv20*Xu5P*((ACETP*GAP)/(K_eqv20*PHI*Xu5P) - 1))/(KmPHIv20*KmXu5Pv20^2*(ACETP/KmACETPv20 + PHI/KmPHIv20 + 1)*(GAP/KmGAPv20 + Xu5P/KmXu5Pv20 + 1)^2*(E4P/KmE4Pv20 + F6P/KmF6Pv20 + 1)) - (PHI*V_maxv20*((ACETP*GAP)/(K_eqv20*PHI*Xu5P) - 1))/(KmPHIv20*KmXu5Pv20*(ACETP/KmACETPv20 + PHI/KmPHIv20 + 1)*(GAP/KmGAPv20 + Xu5P/KmXu5Pv20 + 1)*(E4P/KmE4Pv20 + F6P/KmF6Pv20 + 1)) + (ACETP*GAP*V_maxv20)/(K_eqv20*KmPHIv20*KmXu5Pv20*Xu5P*(ACETP/KmACETPv20 + PHI/KmPHIv20 + 1)*(GAP/KmGAPv20 + Xu5P/KmXu5Pv20 + 1)*(E4P/KmE4Pv20 + F6P/KmF6Pv20 + 1));
...
% Row 20
dfodc(20,19)= (F6P*PHI*V_maxv19*((ACETP*E4P)/(F6P*K_eqv19*PHI) - 1))/(KmACETPv19*KmF6Pv19*KmPHIv19*(ACETP/KmACETPv19 + PHI/KmPHIv19 + 1)^2*(GAP/KmGAPv19 + Xu5P/KmXu5Pv19 + 1)*(E4P/KmE4Pv19 + F6P/KmF6Pv19 + 1)) - (E4P*V_maxv19)/(K_eqv19*KmF6Pv19*KmPHIv19*(ACETP/KmACETPv19 + PHI/KmPHIv19 + 1)*(GAP/KmGAPv19 + Xu5P/KmXu5Pv19 + 1)*(E4P/KmE4Pv19 + F6P/KmF6Pv19 + 1));
dfodc(20,20)= (PHI*V_maxv20*Xu5P*((ACETP*GAP)/(K_eqv20*PHI*Xu5P) - 1))/(KmACETPv20*KmPHIv20*KmXu5Pv20*(ACETP/KmACETPv20 + PHI/KmPHIv20 + 1)^2*(GAP/KmGAPv20 + Xu5P/KmXu5Pv20 + 1)*(E4P/KmE4Pv20 + F6P/KmF6Pv20 + 1)) - (GAP*V_maxv20)/(K_eqv20*KmPHIv20*KmXu5Pv20*(ACETP/KmACETPv20 + PHI/KmPHIv20 + 1)*(GAP/KmGAPv20 + Xu5P/KmXu5Pv20 + 1)*(E4P/KmE4Pv20 + F6P/KmF6Pv20 + 1));
...
dfodc(22,23)= (NADP*V_maxv23*(NADPH/(K_eqv23*NADP) - 1))/(KmNADPv23^2*(NADP/KmNADPv23 + NADPH/KmNADPHv23 + 1)^2) - (V_maxv23*(NADPH/(K_eqv23*NADP) - 1))/(KmNADPv23*(NADP/KmNADPv23 + NADPH/KmNADPHv23 + 1)) + (NADPH*V_maxv23)/(K_eqv23*KmNADPv23*NADP*(NADP/KmNADPv23 + NADPH/KmNADPHv23 + 1));
